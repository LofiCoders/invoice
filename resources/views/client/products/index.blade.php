                                ...
                                @forelse($products as $product)
                                    <tr>
                                        ...
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                            @if($product->type == 1)
                                                <span class="inline-flex items-center rounded-full bg-yellow-100 px-3 py-0.5 text-sm font-medium text-yellow-800">Service</span>
                                            @else
                                                <span class="inline-flex items-center rounded-full bg-green-50-100 px-3 py-0.5 text-sm font-medium text-green-800">Product</span>
                                            @endif
                                        </td>
                                        <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">{{config('saas.currency')}} {{number_format($product->price/100)}}</td>
                                        ...
                                        <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 inline-flex">
                                            @can('product_edit')
                                                <a href="{{ route('account.products.edit',$product->id)}}"
                                            ...
                                            @endcan
                                            @can('product_delete')
                                                <form action="{{ route('account.products.destroy',$product->id)}}"
                                                      method="POST">
                                                    @csrf
                                                    @method('DELETE')
